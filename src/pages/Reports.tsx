...
const reports = [
  ...
  { title: 'Relatório de Pagamentos', desc: 'Status de pagamentos e inadimplência', icon: FileText },
];
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-base mt-1">Gere relatórios do seu negócio</p>
      </div>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {reports.map(r => (
          <Card key={r.title} className="glass-card border-0 hover:bg-card/60 transition-colors">
